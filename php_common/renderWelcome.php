...
$table = "<table class='table table-hover table-white' id='managedTrip'>
<thead>
    <tr>
...
{
    $today = date("Y-m-d");
    echo "<h1 class='text-center'>Welcome! $GLOBALS[position], $GLOBALS[login_session] </h1>";
    switch ($GLOBALS['position']) {
        case 'Manager':
            echo $GLOBALS['table'];
            $query_sql = mysqli_query($GLOBALS['db'], "SELECT B.Booking_ID,C.FName,C.LName,Trip.Trip_ID,T.TourCode,C.Phone_num,T.Name,T.Destination,Trip.Departure_date,Trip.Fee,Trip.Airline,E.username ,T.itinerary_url FROM Booking B INNER JOIN Customer C ON B.FK_C_username=C.username INNER JOIN Trip on Trip.Trip_ID=B.FK_Trip_ID INNER join Tour T on Trip.FK_TourCode=T.TourCode INNER JOIN Employee E on T.FK_E_username=E.username WHERE Trip.Departure_date <= $today ");
            while ($row = mysqli_fetch_assoc($query_sql)) {
                ...
                <td>RM {$row['Fee']}</td>
                <td>{$row['Airline']}</td>
                <td><a href='{$row[' itinerary_url']}'><img src='img/itenerary-dark.png' /></a> </td>
            </tr>";
            }
            echo "</tbody></table>";
            break;
        ...
        default:
            ...
            while ($row = mysqli_fetch_assoc($query_sql)) {
                ...
                echo "<tr >
                <td >{$row['FName']} {$row['LName']}</td>
                <td id='customerPhone$ran' data-id='{$row['username']}'>{$row['Phone_num']}</td>
                <td>{$row['TourCode']}</td>
                <td>{$row['Name']}</td>
                <td>{$row['Destination']}</td>
                <td>{$row['Departure_date']}</td>
                ...
                <td>{$row['Airline']}</td>
                <td><a href='{$row[' itinerary_url']}'><img src='img/itenerary-dark.png' /></a> </td>
                <td><a class='btn btn-primary text-white' role='button' onclick='makeUpdate(\"$ran\")' id='btn_$ran'>Update</a></td>
            </tr>
            ";
            }
            echo "</tbody></table>";
